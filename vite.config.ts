...
function normalizeApiTarget(raw: string): string {
...
export default defineConfig(({ mode }) => {
  ...
  const apiTarget = normalizeApiTarget(env.VITE_API_URL || 'http://localhost:3000')
...
function figmaSiteConfiguration(config: FigmaSiteConfiguration): Plugin {
  ...
  const title = config.title ?? "Tafakuri-Think With Ndomi"
